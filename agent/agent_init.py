...
def init_agent(
    ...
    # [AOMS-REFINE S4, Pi] Opus output-budget cap (max_tokens->4096; effort kept xhigh).
    # Opus-only; free/local models unchanged. Effort cap DISABLED per Kyle
    # 2026-06-19 - Opus keeps full xhigh reasoning depth; only max_tokens is capped.
    try:
        from agent.auxiliary_client import _output_budget_override_for_model as _obom
        _mt_ov, _rc_ov = _obom(agent.model, agent.max_tokens, agent.reasoning_config)
        if _mt_ov is not None:
            agent.max_tokens = _mt_ov
        if _rc_ov is not None:
            agent.reasoning_config = _rc_ov
    except Exception:
        pass

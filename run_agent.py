...
# ── OMNIRA Immune Guard (shadow mode by default) ────────────────────────
try:
    from hermes_guard import immune_check
except ImportError:
    immune_check = None  # graceful degradation if guard file missing
...
class AIAgent:
        ...
        def _call():
            try:
                if self.api_mode == "codex_responses":
                    ...
                    # ── OMNIRA Immune Checkpoint ───────────────────
                    if immune_check is not None:
                        immune_check(api_kwargs.get("messages", []),
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # ── OMNIRA Immune Checkpoint ───────────────────────────
            if immune_check is not None:
                immune_check(stream_kwargs.get("messages", []),
                             getattr(self, 'provider', 'unknown'),
                             getattr(self, 'model', 'unknown'))
...
# Check approval before touching activity    if check_approval(self.agent_id):        self._touch_activity(            f"waiting for stream response ({_waiting_secs}s, no chunks yet)"        )    else:        print("Heartbeat approval denied. Pausing activity.")
                )

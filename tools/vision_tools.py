...
Nous, Codex, native Anthropic, or a custom OpenAI-compatible endpoint. The
registered ``vision_analyze`` tool prefers native image attachment to the active
main model when supported, then falls back to auxiliary vision, then Pi-hosted
Ollama/Moondream as a local last resort.
...
def _should_use_native_vision_fast_path() -> bool:
    ...
    try:
        ...
        # `agent.image_input_mode: text` is an explicit user veto: do not attach
        # pixels to the main model from this tool.
        agent_cfg = cfg.get("agent") if isinstance(cfg, dict) else {}
        if isinstance(agent_cfg, dict):
            if str(agent_cfg.get("image_input_mode") or "").strip().lower() == "text":
                return False

        supports_override = _lookup_supports_vision(provider, model, cfg) is True
        if supports_override:
            # Explicit model/support metadata means the active model can see.
            # Do not let an auxiliary.vision provider in `auto` mode silently
            # downgrade vision_analyze to text/Pi when the chat substrate itself
            # can inspect the pixels.
            return True
...
async def _vision_analyze_pi(
    ...
    try:
        ...
        if image_source.lower().startswith("data:image/"):
            ...
            image_size_bytes = temp_image_path.stat().st_size
            detected_mime_type = _detect_image_mime_type_from_bytes(
                temp_image_path.read_bytes()
            )
...
def check_vision_tool_available() -> bool:
    """Return True when any registered vision path can serve the tool."""
    return (
        _should_use_native_vision_fast_path()
        or check_vision_requirements()
        or check_pi_vision_requirements()
    )
...
VISION_ANALYZE_SCHEMA = {
    "name": "vision_analyze",
    "description": (
        "Load/analyze an image. Accepts a URL, local file path, or data URL. "
        ...
        "When the active model supports native vision, the image is attached "
        "directly to the current model context; otherwise it falls back to "
        "configured auxiliary vision, then Pi-local Moondream as a last resort."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
async def _handle_vision_analyze(args: Dict[str, Any], **kw: Any) -> str:
    ...
    # Text fallback: use the configured auxiliary vision router first. Pi-local
    # Moondream remains a last-resort fallback when no auxiliary vision client is
    # available, not the default route for vision-capable sessions.
    ...
    if check_pi_vision_requirements():
        pi_model = os.getenv("HERMES_PI_VISION_MODEL", "").strip() or None
        return await _vision_analyze_pi(image_url, full_prompt, pi_model)

    return await vision_analyze_tool(image_url, full_prompt, model=None, task_id=task_id)
...
registry.register(
    ...
    check_fn=check_vision_tool_available,
    is_async=True,
    emoji="👁️",
)
